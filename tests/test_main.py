...
def test_search(browser):
    ...
    MainPage(browser)._click_element(MainPage.SEARCH_BUTTON)
    assert MainPage(browser)._get_element_attribute(MainPage.SEARCH_TITLE, "innerHTML") == "Search"


def test_cart_empty(browser):
    """Check that the shopping cart is empty"""
    MainPage(browser)._click_element(MainPage.CART_BUTTON)
    assert MainPage(browser)._get_element_attribute(MainPage.EMTY_CART_DROPDOWN_ITEM, "innerHTML") ==\
           "Your shopping cart is empty!"


def test_add_to_wishlist(browser):
    """Check that the alert appears after adding an item to wishist"""
    MainPage(browser)._click_element(MainPage.ADD_TO_WISHLIST_BUTTON)
    time.sleep(0.3)
    assert MainPage(browser)._find_element(SuccessAlert.SUCCESS_ALERT)
...
def test_my_account_dropdown_menu(browser):
    ...
    assert MainPage(browser)._find_element(MainPage.REGISTER_DROPDOWN_ITEM)
    assert MainPage(browser)._find_element(MainPage.LOGIN_DROPDOWN_ITEM)
